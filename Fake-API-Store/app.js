...
burgerBtn.addEventListener('click', function(){
    ...
    burgerMenu.style.transition = 'height 0.5s, opacity .5s linear';
   
    setTimeout(()=>{
        squeezeBurger(burgerCloseBtnSpan, '1px', 'height 0.5s linear')
    }, 1000);
    
    setTimeout(()=>{
        makeX(lineOne,lineTwo,lineThree,'transparent','rotate, 0.5s','rotate(45deg)','rotate(-45deg)')
    }, 2000);

    setTimeout(()=>{
        enableClose(burgerCloseBtn,burgerBtn,'')
    },2500);
});
burgerCloseBtn.addEventListener('click', function(){
    ...
    setTimeout(()=>{
        deSqueezeBurger(lineTwo,burgerCloseBtnSpan,'black','100%','height 0.5s linear');
    },500);

   
    setTimeout(()=>{
        closeMenu(burgerMenu,'0','0vh','height 0.5s opacity 0.5s linear');
    },1500);


    setTimeout(()=>{
        collapseMenu(burgerMenu,'collapse');
    },2000);
});
...
cartBtn.addEventListener('click',function(){
    ...
    setTimeout(()=>{
        squeezeBurger(cartCloseBtn, '1%', 'height 0.5s linear')
    }, 1000);
    
    setTimeout(()=>{
        makeX(lineOneCart,lineTwoCart,lineThreeCart,'transparent','rotate, 0.5s','rotate(45deg)','rotate(-45deg)')
    }, 2000);

    setTimeout(()=>{
        enableClose(cartCloseBtn,cartCloseBtn,'1%')
    },2500);

    cartMenu.style.visibility = 'visible'
    cartMenu.style.opacity = '1';
    cartMenu.style.width = '100%';
    cartMenu.style.transition = 'width 0.5s, opacity .5s linear';
});
 ...
 cartCloseBtn.addEventListener('click',function(){
    ...
    setTimeout(()=>{
        deSqueezeBurger(lineTwoCart,cartCloseBtn,'black','20%','height 0.5s linear');
    },500);

   
    setTimeout(()=>{
        closeMenuWidth(cartMenu,'0','0vw','height 0.5s opacity 0.5s linear');
    },1500);


    setTimeout(()=>{
        collapseMenu(cartMenu,'collapse');
    },2000);
...
function addProductsToGrid(products){
    ...
    for(let i = 0; i< products.length; i++){
        ...
        div.addEventListener('click',()=>{
            ...
            setTimeout(()=>{
                squeezeBurger(popUpCloseBtn, '1px', 'height 0.5s linear')
            }, 1000);
    
            setTimeout(()=>{
                makeX(popUpLineOne,popUpLineTwo,popUpLineThree,'transparent','rotate, 0.5s','rotate(45deg)','rotate(-45deg)')
            }, 2000);

            setTimeout(()=>{
                enableClose(popUpCloseBtn,popUpCloseBtn,'')
            },2500);
    ...
    if(window.location.pathname.includes('products.html')){
        ...
        popUpBuyBtn.addEventListener('click',()=>{
            ...
            thumbnailIMG.setAttribute('src', imagePath);
            textItem.textContent = 'Item description';
            quantityInt.textContent = '1';
            price.textContent = '$'+priceFromDiv.textContent;
            ...
            quantity.addEventListener('click', event => {
                if (event.target.classList.contains('subtract')) {
                    ...
                    price.textContent = `$${(change * parseFloat(priceFromDiv.textContent.replace('$',''))).toFixed(2)}`
...
function makeDropDown(categories){
    ...
    console.log(ulDropdownCategory)
...
if(window.location.pathname.includes('check-out.html')){
    ...
    div = localStorage.getItem('myContent');
    let data= document.createElement('div');
    data.innerHTML = div;

    let product = {};
    product.description = data.querySelector('[attribute="product-description"]').textContent;
    product.name = data.querySelector('[attribute="product-name"]').textContent;
    product.price = data.querySelector('[attribute="product-price"]').textContent;
    product.id = data.querySelector('[attribute="product-id"]').textContent;
    product.rating = data.querySelector('[attribute="product-rating"]').textContent;
    product.category = data.querySelector('[attribute="product-category"]').textContent;
    product.image = data.querySelector('[attribute="product-image"]').textContent;

    let productImage  = product.image;
    let img = document.querySelector('.confirm-product .image-check-out img');
    img.setAttribute('src',productImage);

    let text = product.name;
    let productName = document.querySelector('.confirm-product .name p');
    productName.textContent = text;

    let price = product.price;
    let productPrice = document.querySelector('.confirm-product .price p');
...
$(document).ready(function() {
    $(function() {
        ...
        function checkFirstname() {
            let pattern =  /^[a-zA-ZåäöÅÄÖ]{2,50}\s[a-zA-ZåäöÅÄÖ0-9\s]{0,50}$/;
            ...
            if(pattern.test(firstname) && firstname !== ''){
                ...
                $("#fname-error-message-check-out").html("Should be on form: firstname lastname");
...
if(window.location.pathname.includes('confirmation')){
    ...
    let div = localStorage.getItem('myContent');
    ...
    let template = document.querySelector('.template-confirmation');

    let clone = document.importNode(template.content, true);

    let idItem = clone.querySelector('.name');
    let priceItem = clone.querySelector('.price');
    let quantityItem = clone.querySelector('.quantity');
    let totalAmountItem = clone.querySelector('.total-amount');
    ...
    pTotal.innerHTML = product.price; 

    idItem.appendChild(pId)
    priceItem.appendChild(pPrice)
    quantityItem.appendChild(pQuantity);
    totalAmountItem.appendChild(pTotal);

    products.appendChild(idItem);
    products.appendChild(priceItem);
    products.appendChild(quantityItem)
    products.appendChild(totalAmountItem);

    let subTotal = document.querySelector(".total-cost p")
    subTotal.innerHTML = subTotal.textContent+product.price
